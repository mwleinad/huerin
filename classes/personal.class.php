...
class Personal extends Main
{
    ...
    {
        $this->Util()->DB()->setQuery("SELECT a.personalId, a.name, a.roleId, b.name as nameRol, b.nivel, a.sueldo, a.fechaIngreso, a.jefeInmediato, a.departamentoId,
                                             CASE 
    ...
    public function getPersonasParaDepartamentoGerencial() {
        ...
        $sql = "SELECT 
                    ...
                    WHERE 
                        (
                            roles.nivel IN (3,5) 
                            AND (SELECT tbl_dep.departamento FROM departamentos tbl_dep WHERE tbl_main.departamentoId = tbl_dep.departamentoId LIMIT 1) NOT IN ('Contabilidad e Impuestos', 'Auditoria', 'Fiscal','Gestoria', 'Legal')
                        ) 
                         OR 
                        (
                            (roles.nivel >=3 and roles.nivel <=4) 
                            AND (SELECT tbl_dep.departamento FROM departamentos tbl_dep WHERE tbl_main.departamentoId = tbl_dep.departamentoId LIMIT 1) IN ('Contabilidad e Impuestos', 'Auditoria', 'Fiscal', 'Gestoria', 'Legal')
                        )
