...
class Override extends Comprobante
{
	function GeneratePDF($data, $serie, $totales, $nodoEmisor, $nodoReceptor, $nodosConceptos,$empresa, $cancelado = 0, $vistaPrevia = 0)
	{
  	...
  	$pdf->Cell(20,10,$data["comprobante"]["nombre"],0,0,'C');
		...
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(80,3,utf8_decode(urldecode($data["nodoEmisor"]["rfc"]["municipio"]." ".$data["nodoEmisor"]["rfc"]["pais"]." ".$data["nodoEmisor"]["rfc"]["estado"]." CP: ".$data["nodoEmisor"]["rfc"]["cp"])),0);
		...
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(70,3,"Sello",0);

		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(70,3,"Sello PAC",0);
		...
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(43,3,"00001000000202639096",0,0,'R');
		...
		foreach($data["nodoReceptor"] as $key => $value)
		{
			$data["nodoReceptor"][$key] = trim($value);
		}
...
//		$pdf->CheckPageBreak(50);
		...
		switch($data["tiposDeMoneda"])
		{
			...
			case "dolar": $tiposDeCambio = "Dolares"; $tiposDeCambioSufix = "USD";break;
...
//echo $data["formaDePago"];
		...
		$moneda = "$ ";
		if($data["tiposDeMoneda"] == "euro")
		{
			$moneda = "EUR ";
		}
		if($data["tiposDeMoneda"] == "dolar")
		{
			$moneda = "USD ";
		}
		//block totales
...
//		$pdf->Ln();
		...
		{
			$pdf->Output(DOC_ROOT."/pdf/vistaPrevia.pdf", "F");
		}
