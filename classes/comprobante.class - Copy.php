...
//			$card['ivaTotal'] = $val["ivaTotal"];
	...
	function SearchComprobantesByRfc($values){
		...
		$sqlQuery = 'SELECT *, c.status AS status FROM comprobante AS c 
	...
	public function SendComprobante($id_comprobante){
		...
		$mail->SetFrom("facturacion@braunhuerin.com.mx", "Administrador del Sistema");
...
function Header()
{
    ...
    $this->SetFont('Arial','B',10);
		$this->SetY(0);
		$this->SetX(10);
    $this->Cell(190,10,'Comprobantes Fiscales Digitales http://www.facturase.com',0,0,'C');
    //Line break
    $this->Ln(20);
}
