...
class AutomaticInvoiceRif extends Comprobante
{
...
<?php
	...
	foreach($data["contratosHuerin"] as $key => $contratoHuerin)
	{
		...
		WHERE tipoServicioId = '".RIF."' AND contractId = '".$contratoHuerin["contractId"]."'");
...
<?php
echo "remover esto para que se creen";
exit;
		...
		//	echo "jere";
			...
			foreach($servicios as $res){
				...
				$_SESSION["conceptos"][] = array(
					...
					"excentoIva" => "no",
					"descripcion" => $res["nombreServicio"],
					"tasaIva" => $tasaIva
				);
