INSERT INTO `permisos` (`titulo`, `parentId`, `namePage`, `levelDeep`) VALUES ('Cobranza detallado', 7, 'report-cobranza-detallado', 1);
...
) empresas ON comprobante.userId = empresas.id WHERE year(comprobante.fecha) = anio AND tiposComprobanteId = 1;
...
FROM comprobante INNER JOIN (
    ...
    FROM contract
             INNER JOIN customer ON contract.customerId =  customer.customerId
) empresas ON comprobante.userId = empresas.id WHERE year(comprobante.fecha) = anio AND tiposComprobanteId = 1 INTO vTotalRow;
