...
else
{
	  	...
	  	foreach ($jefeSoc as $key => $soc) {
		 	...
		 	$db->setQuery("SELECT t.tipoServicioId,t.nombreServicio,i.instanciaServicioId
			                ...
			                where cc.activo = 'Si' and s.status = 'activo' and i.status!='completa' and cc.responsableCuenta in (
			                	SELECT personalId
								FROM personal
								where jefeSocio='".$soc[personalId]."')
			...
			foreach ($servicios as $key => $serv) {
			    ...
			    $db->setQuery("SELECT personalId,name
								FROM personal
								where jefesocio='".$soc[personalId]."' and personalId in(
										select personalId
							            FROM contract
							            left join personal on responsableCuenta = personal.personalId
							            where activo = 'Si'
			    ...
			    foreach ($responsablePDF as $key => $res) {
					...
					$html[$key] = '<html>
						                ...
						                <td width="20%">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
						                <td width="60%" align="left">'.$soc['name'].'<br>'.$serv['nombreServicio'].'<br>'.date('d/m/Y').'<br>'.$subject.'</td>
						                </tr>
						                </table>
						                <br><br><br>
				...
				$fromName = "Administrador del Sistema";
	      		...
	      		foreach ($html as $key => $value) {
					...
					$dompdf->render();
					file_put_contents("reporte_semanal_socio_".($key+1).".pdf", $dompdf->output());
					$attachment = DOC_ROOT."/cron/reporte_semanal_socio_".($key+1).".pdf";
					$fileName = "reporte_semanal_socio_".($key+1).".pdf";
				...
				echo "Email Enviado al Socio ".$toName;
				echo "<br>";
			}
		}
	...
	$file = DOC_ROOT."/cron/reportSemanalSocio.txt";
	$open = fopen($file,"w");
