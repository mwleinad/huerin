      ...
      session_start();
        ...
        $db->setQuery('
            SELECT
                ...
                p.email,
                p.departamentoId,
                (select S.name from personal S where p.jefeSocio = S.personalId) as nameSocio,
                (select S.email from personal S where p.jefeSocio = S.personalId) as emailSocio,
                (select S.personalId from personal S where p.jefeSocio = S.personalId) as personalIdSocio
            FROM
                personal AS p
            WHERE
                p.personalId IN (
        ...
        foreach ($jefesGerente as $keyJG => $rowJG) {
            ...
            <table border="0" cellspacing="0" cellpadding="0" position="fixed">
                ...
                $html  .= '<td align="left" '.$color.'>';
                        ...
                        $DATO[$serv['tipoServicioId']]['name'] = $rowJG['nameSocio'];
                        $DATO[$serv['tipoServicioId']]['email'] = $rowJG['emailSocio'];
                        $DATO[$serv['tipoServicioId']]['personalId'] = $rowJG['personalIdSocio'];
