	...
	if (!isset($_SESSION))
	{
		...
		$db->setQuery("
			...
			WHERE
				jefeContador in (
					SELECT
						GROUP_CONCAT(p.personalId)
					FROM
						personal as p
					WHERE
						p.personalId in (
								SELECT
									personal.jefeContador
								FROM
									contract
				...
				email like '%@%'
		");
		$AUXILIARArray = $db->GetResult();

		foreach ($AUXILIARArray as $key => $auxiliar) {
			...
			$db->setQuery("
				...
				WHERE
					...
					/* i.status!='completa' AND */
					cc.responsableCuenta = '".$auxiliar['personalId']."'
			...
			foreach ($servicios as $key => $serv) {
				...
				$db->setQuery("
					...
					FROM personal
					where
					personalId='".$auxiliar[personalId]."'
				");
				...
				$db->setQuery("
					...
					WHERE
						...
						s.status = 'activo' AND
						i.status!='completa' AND
						cc.responsableCuenta = '".$auxiliar['personalId']."' AND
								...
								<body>
									<table border="0" cellspacing="0" cellpadding="0">
										<tr>
											...
											<td width="20%">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
											<td width="60%" align="left">'.$auxiliar['name'].'<br>'.$serv['nombreServicio'].'<br>'.date('d/m/Y').'<br>'.$subject.'</td>
										</tr>
									</table>
									<br><br><br>
				...
				echo "Email Enviado al Auxiliar ".$toName;
				echo "<br>";
			}
		}
