...
class Pac33 extends Util
{
    ...
    function GetCfdi($xmlConSello)
    {
        ...
        @unlink($xmlConSello['zipFile']);
        $this->Util()->Zip($xmlConSello['root'], $xmlConSello['fileName']);

        //open zip and encode it
        $fh = fopen($xmlConSello['zipFile'], 'r');
        $theData = fread($fh, filesize($xmlConSello['zipFile']));
        $zipFileEncoded = base64_encode($theData);
        fclose($fh);
