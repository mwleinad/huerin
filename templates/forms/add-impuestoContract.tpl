  	...
  	<input name="contractId" id="contractId" type="hidden" value="{$contractId}" size="50"/>
		<fieldset>
			<div class="formLine" style="width:100%; text-align:left">
				<div style="width:30%;float:left">Tipo de Impuesto:</div>
        <select name="impuestoId" id="impuestoId">
        {foreach from=$impuestos item=item}
        	<option value="{$item.impuestoId}">{$item.impuestoNombre}</option>
        {/foreach}
        </select>
			</div>
