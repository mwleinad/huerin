<table width="100%" cellpadding="0" cellspacing="0" class="box-table-a" id="infoGral">
    ...
    {if in_array(223,$permissions) || $User.isRoot}
        <tr>
            ...
            <td align="left"><input name="nameRepresentanteLegal" id="nameRepresentanteLegal" type="text"
                                    value="{$bse.nameRepresentanteLegal}" class="smallInput medium" size="50"/></td>
        </tr>
    {/if}
    <tr>
                ...
                {foreach from=$empleados item=item}
                    {if $item.name neq '.' && $item.departamentoId eq 1}
                        <option value="{$item.personalId}"
                                {if $allPerm[1] eq $item.personalId}selected{/if}>{$item.name}</option>
                    {/if}
                {/foreach}
            </select>
        </td>
