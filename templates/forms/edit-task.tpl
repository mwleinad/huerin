				...
				<div style="display: table-cell;width: 70%;vertical-align: middle;">
					<table>
						...
						{foreach from=$extensiones key=kexp item=exp}
							{if $ncol eq 4}
								<tr><td><input type="checkbox" name="extensiones[]" id="extensiones" value="{$exp.mime}" {if $exp.permitido}checked{/if} ></td><td>{$exp.name}</td>
								{assign var='ncol' value=$ncol-1}
							{else}
								<td><input type="checkbox" name="extensiones[]"  id="extensiones" value="{$exp.mime}" {if $exp.permitido}checked{/if} ></td> <td>{$exp.name}</td>
								{assign var='ncol' value=$ncol-1}
								{if $ncol eq 0}
									{assign var='ncol' value=4}
									</tr>
