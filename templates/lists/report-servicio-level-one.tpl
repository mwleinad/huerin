...
{foreach from=$clientes item=item key=key name=levelOne}
    <ul  class="jstree-container-ul jstree-children">
        <li class="jstree-node jstree-closed {if $smarty.foreach.levelOne.last}jstree-last{/if}" id="before-level2-{$item.nameContact}"><i class="jstree-icon jstree-ocl" role="presentation"></i>
           ...
           <ul id="level2-{$item.nameContact}"  class="jstree-children" role="group">
               {foreach from=$item.razones item=razon  key=keyRazon name=levelRazones}
                <li class="jstree-node jstree-closed {if $smarty.foreach.levelRazones.last}jstree-last{/if}" role="treeitem" id="before-level3-{$razon.rfc}"><i class="jstree-icon jstree-ocl" role="presentation"></i>
                    ...
                    <ul id="level3-{$razon.rfc}" class="jstree-children" role="group">
                         {foreach from=$razon.servicios item=servicio key=keyServicio name=levelServicios}
                             <li class="jstree-node jstree-closed {if $smarty.foreach.levelServicios.last}jstree-last{/if}" role="treeitem" id="before-level4-{$servicio.tipoServicioId}-{$servicio.servicioId}"><i class="jstree-icon jstree-ocl" role="presentation"></i>
                                 <a  href="javascript:;" onclick="showLevel('level4','{$servicio.tipoServicioId}-{$servicio.servicioId}')" class="jstree-anchor">
                                   ...
                                   <ul id="level4-{$servicio.tipoServicioId}-{$servicio.servicioId}" class="jstree-children" role="group">
                                       {foreach from=$servicio.instanciasXservicio item=it key=keyInstancia name=levelInsServ}
                                           {if $it ne ""}
                                            <li class="jstree-node jstree-closed" id="before-level5-{$it.instanciaServicioId}"><i class="jstree-icon jstree-ocl" role="presentation"></i>
                                                ...
                                                <ul id="level5-{$it.instanciaServicioId}" class="jstree-children" role="group">
                                                    <li class="jstree-node jstree-closed jstree-last" id="before-level6-{$it.instanciaServicioId}"><i class="jstree-icon jstree-ocl" role="presentation"></i>
