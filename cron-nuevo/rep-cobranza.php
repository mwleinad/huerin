...
$sql = "SELECT a.paymentDate,concat_ws('',b.serie,b.folio) as factura,a.amount as importe,a.deposito,d.nameContact,c.name,e.name as responsable,
        ...
        WHERE b.empresaId=21 AND date(a.paymentDate)>=DATE_ADD(CURDATE(),INTERVAL -0 MONTH) - INTERVAL DAYOFMONTH(DATE_ADD(CURDATE(),INTERVAL -0 MONTH)) - 1 DAY
        AND date(a.paymentDate) <= LAST_DAY(DATE_ADD(CURDATE(),INTERVAL -0 MONTH)) ORDER BY  a.paymentDate DESC ";
$db->setQuery($sql);
$payments = $db->GetResult($sql);
$db->setQuery('SELECT DATE_ADD(CURDATE(),INTERVAL -0 MONTH) - INTERVAL DAYOFMONTH(DATE_ADD(CURDATE(),INTERVAL -0 MONTH)) - 1 DAY from payment where 1');
