...
switch($_POST['type']){
    ...
    case 'layout-recotizar-servicios':
        ...
        foreach($servicios as $key=>$value) {
            ...
            foreach($puestos as $puesto) {
                $serv[$puesto['name']] = $jefes[$puesto['name']] ?? '';
            }

            if(isset($rolRes['nameLevel'])) {
    ...
    case 'layout-inventario':
        ...
        $tiposSoftware    = ['Aspel COI','Aspel NOI','Aspel Facture','Admin XML','Licencia de Windows', 'Office'];
        $tiposRecurso     = ['Accesorios','Computadora','Sistemas'];
        $tiposDispositivo = ['Monitor','Hdmi','Cable VGA','Mouse','Teclado','Mousepad','Ventilador','Adicionales'];
        ...
        $sheet->setCellValueByColumnAndRow($col,1,'Tipo sofware');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'Procesador');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'Velocidad Procesador');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'Memoria RAM');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,"Tipo de RAM");
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'Disco Duro');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'Tipo de Almacenamiento');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'No Fisico');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'Estatus');
        $col++;
        $sheet->setCellValueByColumnAndRow($col,1,'Ubicacion');
        $col++;
    ...
    case 'layout-reporte-recotizar':
        ...
        $row=2;
        $coorVerticalTotalPrecioCartera = [];
