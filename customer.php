...
switch($_POST["type"])
{
	case "search": 
			...
			//$result = $customer->Search("subordinado", $_POST["tipo"]);
			$result = $customer->SuggestCustomerCatalog($_POST["valur"], $type = "subordinado", $customerId = 0, $_POST["tipo"]);
	...
	case "saveAddCustomer":
			...
			else
			{
				...
				$customers = $util->EncodeResult($resCustomers);
...
//				$resCustomers = $customer->Enumerate();
	...
	case "editCustomer":
			...
			$smarty->assign("DOC_ROOT", DOC_ROOT);
			$customer->setCustomerId($_POST['customerId']);
			$myCustomer = $customer->Info();
			
			$info = $util->EncodeRow($myCustomer);
